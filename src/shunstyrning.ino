...
const char sm2r3[]  = "";
const char sm2r4[]  = "";
const char sm3r1[]  = "Ange antal";
const char sm3r2[]  = "@ Sek";
const char sm3r3[]  = "";
const char sm3r4[]  = "";
const char sm4r1[]  = "Ange antal";
const char sm4r2[]  = "Min -> Max";
const char sm4r3[]  = "@ Sek";
const char sm4r4[]  = "";
const char sm5r1[]  = "Ange antal";
...
void errorView()
{
    ...
    char outstring[] ="*FEL*TEMPGIVARE*FEL*";
    char outtemp[4];
    char intemp[4];
    String s;
    ...
    if (currOutSideTemp < -126){
        ...
        strcpy(outstring,utegivare);
        strcat(outstring, " FEL");
    } else {
        memset(outstring, 0, sizeof(outstring));
        strcpy(outstring,utegivare);
    ...
    Printstring(1,0,outstring );
     if (currPipeTemp < -126){
         memset(outstring, 0, sizeof(outstring));
         strcpy(outstring,framledning);
         strcat(outstring, " FEL");
     } else {
         memset(outstring, 0, sizeof(outstring));
         strcpy(outstring,framledning);
         strcat(outstring,intemp);
         strcat(outstring,";C");
     }
    Printstring(2,0,outstring);
    memset(outstring, 0, sizeof(outstring));
    strcpy(outstring,"Ger ut MAX temp! ");
    Printstring(3,0,outstring);
}
...
void TempView()
{
    if(!inMainView){lcd.clear();};
    inMainView=true;
    SetCurrTemp();
    outstr ="Ute:"+(String)currOutSideTemp+";C :Fram"+(String)currPipeTemp+";C";
    Printstring(0,0,outstr);
...
void AdjustView(bool upp)
{
    if(inMainView){
        ...
        outstr =emptyRow;
        Printstring(1,0,outstr);
        outstr ="Fram"+(String)currPipeTemp+";C -> "+(String)currCalcTemp+";C";
        Printstring(2,0,outstr);
        outstr =emptyRow;
        Printstring(3,0,outstr);
    }
